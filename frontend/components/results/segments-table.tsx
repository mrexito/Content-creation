          ...
          <div key={i} className="border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span className="text-sm font-semibold text-gray-800">Segment {i + 1}</span>
                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${DOMAIN_STYLES[domain] ?? DOMAIN_STYLES.general}`}>
                  {domain}
                </span>
              </div>
              <span className={`text-xs font-medium shrink-0 ml-2 ${
                validCount === totalCount ? "text-emerald-600" : "text-amber-600"
              }`}>
            ...
            {isOpen && (
              <div className="p-4 space-y-4 bg-gray-50 border-t border-gray-200">
                ...
                {seg.validated_variants?.map((variant, vi) => (
                  <div key={vi} className="border border-gray-200 rounded-lg p-3 bg-white space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-medium text-gray-500">Variante {vi + 1}</span>
                      {variant.is_valid ? (
