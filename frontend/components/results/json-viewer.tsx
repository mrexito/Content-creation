...
function JsonNode({ data, depth = 0 }: { data: unknown; depth?: number }) {
  ...
  if (data === null) return <span className="text-slate-300">null</span>
    ...
    return (
      <span>
        ...
        >
          ...
          <span className="text-slate-300 text-xs ml-0.5">[{data.length}]</span>
        </button>
        {!collapsed && (
          <div className="ml-4 border-l border-slate-500 pl-3">
            {data.map((item, i) => (
          ...
          <div className="ml-4 border-l border-slate-500 pl-3">
            {keys.map((k) => (
              <div key={k} className="py-0.5">
                <span className="text-purple-300 text-xs">&quot;{k}&quot;</span>
