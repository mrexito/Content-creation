"""
evaluate_all_frameworks.py
==========================
Zentrale Thesis-Evaluation: Alle sechs Prototypen über alle drei Domains.

Frameworks:
  1. langchain          LangChain Chains (sequenziell)
  2. langgraph          LangGraph StateGraph (deklarativer Graph)
  3. hybrid             LangChain Pre/Post + LangGraph Mitte
  4. hybrid_agent       LangChain Pre/Post + AgentExecutor Mitte
  5. agent_orchestrator Ein Agent, alle Tools (ReAct-Pattern)
  6. agent_multi        Multi-Agent Pipeline (ein Agent pro Tool)

Usage:
    python scripts/evaluate_all_frameworks.py
    python scripts/evaluate_all_frameworks.py --variants 1 --frameworks langchain langgraph hybrid
    python scripts/evaluate_all_frameworks.py --fast
    python scripts/evaluate_all_frameworks.py --agents-only
    python scripts/evaluate_all_frameworks.py --domains math --frameworks hybrid hybrid_agent
"""
...
def _md_thesis_observations(results: list[dict], frameworks: list[str]) -> str:
    """Generiert 3–5 automatische Beobachtungen für Thesis Kapitel 6."""
    ...
    # 1. Welches Framework erzielt die höchste Validation-Rate pro Domain?
    for domain in ALL_DOMAINS:
    ...
    # 2. Agent Orchestrator vs. LangChain: Zeitvergleich
    for domain in ALL_DOMAINS:
    ...
    # 3. Hybrid+Agent vs. Hybrid: Direkt-Vergleich Phase 2
    ...
    # 4. Agent Multi-Step Degradation
    ...
    # 5. LangGraph conditional edge vs. Agent Retries
    lg_retries = [
...
def _build_json_report(results: list[dict], ts: str, cfg: dict, run_id: int = 1) -> dict:
    return {
        ...
        "config": {
            ...
            "generator":  "evaluate_all_frameworks.py",
        },
        "summary": [
            {
                "run_id":          run_id,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Thesis-Evaluation: Alle sechs Prototypen über alle drei Domains."
    )
    parser.add_argument("--variants",    type=int,  default=1,
                        help="Varianten pro Segment (Standard: 1, für Agents empfohlen)")
    ...
    parser.add_argument("--output-dir",  type=Path, default=None,
                        help="Output-Verzeichnis (Standard: data/output/evaluation/<ts>/)")
    ...
    # ── Laufzeit-Warnung ─────────────────────────────────────────────────────
    print("=" * 70)
    print("  FRAMEWORK-EVALUATION — Alle Prototypen")
    print("=" * 70)
    ...
    if not args.no_markdown:
        md_parts = [
            f"# Framework-Vergleich: Alle Prototypen\n\n"
            f"**Datum:** {ts}  |  **Varianten/Segment:** {args.variants}  |  "
        ...
        md_report = "\n".join(md_parts)
        md_path   = out_dir / "full_comparison_report.md"
