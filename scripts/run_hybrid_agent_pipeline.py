"""
Hybrid Agent Pipeline Runner – für Frontend-Integration

Analog zu run_hybrid_pipeline.py.
Einziger Unterschied: Phase 2 nutzt LangChain AgentExecutor statt LangGraph.

Usage:
    python scripts/run_hybrid_agent_pipeline.py \
        --pdf data/input/math/equations_simple.pdf \
        --domain math \
        --variants 1 \
        --retries 3 \
        --output-dir data/output/hybrid_agent/run_xyz \
        --progress data/output/hybrid_agent/run_xyz/progress.json \
        --run-id run_xyz
"""
import sys
import json
...
def transform_segments(segments_with_variants: list) -> list:
    """Transformiert Hybrid-Agent-Segmente ins Frontend-SegmentResult-Format."""
    result = []
...
def main():
    parser = argparse.ArgumentParser(description="Run Hybrid Agent Pipeline")
        ...
        try:
            ...
            if ocr_data.get("success"):
                ...
                write_progress(progress_path, "error", "error", [], 0, {
                    "pdf_name": args.pdf.name, "run_id": args.run_id,
                    "framework": "hybrid_agent", "domain": args.domain,
    ...
    completed_phases: list = ["parsing"] if pre_parsed_text is not None else []
    ...
    def on_phase_complete(phase: str) -> None:
        ...
        write_progress(
            progress_path, "running", next_phase, list(completed_phases), pct, base_meta
        )
    ...
    try:
        ...
        # Segmente aus assembled_document für Frontend
        segments_raw = []
        ...
        total_valid = agent_stats.get("total_valid", 0)
        total_invalid = agent_stats.get("total_invalid", 0)
        ...
        try:
            ...
            gen = PdfGenerator()
            gen.generate_tasks_pdf(assembled_doc, tasks_path)
            gen.generate_solutions_pdf(assembled_doc, solutions_path)
        ...
        frontend_result: dict = {
            ...
            "metrics": {
                ...
                "validation_rate": agent_stats.get("validation_rate", 0.0),
                # Agent-spezifische Felder für Thesis-Vergleich
                "tool_calls_total": agent_stats.get("total_tool_calls", 0),
                "tool_calls_per_segment": agent_stats.get("tool_calls_per_segment", []),
                "total_attempts": agent_stats.get("total_retries", 0),
        ...
        logger.info(f"✅ Hybrid Agent Pipeline Runner abgeschlossen → {result_path}")
        sys.exit(0)

    except Exception as e:
        logger.error(f"Hybrid Agent Pipeline Runner fehlgeschlagen: {e}")
        write_progress(
            progress_path,
            "error",
            "error",
